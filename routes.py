...
def provider_required(f):
    ...
    Admin users are NOT allowed - this is only for service providers.
...
def download_certificate(cert_id):
    ...
    # Base path for static certificates
    certs_dir = os.path.join(current_app.root_path, 'static', 'certificates')
    pdf_path  = os.path.join(certs_dir, cert.pdf_filename)

    if not os.path.exists(pdf_path):
        # Try finding it in the project root if static folder fails (sometimes seen in dev)
        alt_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'static', 'certificates', cert.pdf_filename)
